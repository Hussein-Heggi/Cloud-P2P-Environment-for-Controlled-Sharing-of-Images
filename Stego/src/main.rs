...
pub fn embed(mut img: DynamicImage, meta: &Meta) -> Result<DynamicImage, StegoError> {
    let json = serde_json::to_vec(meta)?;
    if json.len() > MAX_PAYLOAD_BYTES { return Err(StegoError::TooLarge); }
    ...
    if need_bits > cap_bits { return Err(StegoError::TooLarge); }
